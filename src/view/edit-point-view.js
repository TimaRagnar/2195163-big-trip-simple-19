...
const createEditPointTemplate = (point) => {
  ...
  const pointTypeItemTemplate = offersByTypes
    .map(
      (element) =>
  ...
  offer.type
}-1" type="checkbox" name=${offer.title} ${
  offers.includes(offer.id) ? 'checked' : ''
    ...
    .join('');
  const destinationToChoose = destinations
  ...
  pointDestination.name
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${parceDateStart.format(
    DATE_FORMAT
  )}">
        &mdash;
        <label class="visually-hidden" for="event-end-time-1">To</label>
        <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${parceDateEnd.format(
    DATE_FORMAT
  )}">
      </div>
